...
from lingua_franca.lang.format_gl import pronounce_number_gl
from lingua_franca.lang.parse_common import *
from lingua_franca.lang.common_data_gl import _ARTICLES_GL, _STRING_NUM_GL
...
def extract_numbers_gl(text, short_scale=True, ordinals=False):
    """
    ...
    """
    return extract_numbers_generic(text, pronounce_number_gl,
                                   extract_number_gl, short_scale=short_scale,
                                   ordinals=ordinals)


def normalize_gl(text, remove_articles=True):
    ...
    while i < len(words):
        ...
        if remove_articles and word in _ARTICLES_GL:
            i += 1
            continue

        # Convert numbers into digits
        r = _gl_number_parse(words, i)
        if r:
            v, i = r
            normalized += " " + str(v)
            continue
...
def get_gender_gl(word, context=""):
