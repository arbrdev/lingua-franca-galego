...
from lingua_franca.lang.common_data_gl import _NUM_STRING_GL, \
    _FRACTION_STRING_GL
    ...
    if 20 <= number <= 29:
        ...
        if ones > 0:
            ...
            if ones == 2:
                ...
                result += "e" + _NUM_STRING_GL[ones]
    elif number >= 30:  # do 30 en diante
        tens = int(number-int(number) % 10)
        ones = int(number - tens)
        result += _NUM_STRING_GL[tens]
        if ones > 0:
...
def nice_time_gl(dt, speech=True, use_24hour=False, use_ampm=False):
    ...
    if use_24hour:
        ...
        if hour == 0 or hour == 12:
            ...
            speak = "las " + pronounce_number_gl(hour)
        else:
            speak = "las " + pronounce_number_gl(hour-12)
